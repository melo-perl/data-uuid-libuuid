# This Makefile is for the Data::UUID::LibUUID extension to perl.
#
...
#     INSTALLDIRS => q[site]
#     LIBS => [q[-luuid]]
#     NAME => q[Data::UUID::LibUUID]
#     PL_FILES => {  }
#     PREREQ_PM => { MIME::Base64=>q[0], Sub::Exporter=>q[0], Test::More=>q[0], Test::use::ok=>q[0], asa=>q[0] }
#     SIGN => q[1]
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Data/UUID/LibUUID.pm]
...
H_FILES  = hdr_check.h \
	ppport.h
MAN1PODS = 
MAN3PODS = lib/Data/UUID/LibUUID.pm \
	lib/Data/UUID/LibUUID/DataUUIDCompat.pm
...
DISTNAME = Data-UUID-LibUUID
DISTVNAME = Data-UUID-LibUUID-0.06_01
...
# Data::UUID::LibUUID might depend on some other libraries:
...
.xs$(OBJ_EXT) :
	...
	$(MV) $*.xsc $*.c
	$(CCCMD) $(CCCDLFLAGS) -iwithsysroot "-I$(PERL_INC)" $(PASTHRU_DEFINE) $(DEFINE) $*.c 
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/Data/UUID/LibUUID.pm $(INST_MAN3DIR)/Data::UUID::LibUUID.$(MAN3EXT) \
	  lib/Data/UUID/LibUUID/DataUUIDCompat.pm $(INST_MAN3DIR)/Data::UUID::LibUUID::DataUUIDCompat.$(MAN3EXT) 
...
clean :: clean_subdirs
	- $(RM_F) \
	  ...
	  *$(LIB_EXT) *$(OBJ_EXT) \
	  *perl.core LibUUID.base \
	  LibUUID.bs LibUUID.bso \
	  LibUUID.c LibUUID.def \
	  LibUUID.exp LibUUID.o \
	  LibUUID_def.old MYMETA.json \
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: Data-UUID-LibUUID' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  asa: '\''0'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: 0.06_01' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Data-UUID-LibUUID",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Test::use::ok" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "asa" : "0"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "release_status" : "unstable",' >> META_new.json
	$(NOECHO) $(ECHO) '   "version" : "0.06_01",' >> META_new.json
...
test_static :: subdirs-test_static $(MAP_TARGET)
	PERL_DL_NONLAZY=1 "/Users/melo/projects/cpan/mine/data-uuid-libuuid/$(MAP_TARGET)" $(MAP_PERLINC) "-MExtUtils::Command::MM" "-MTest::Harness" "-e" "undef *Test::Harness::Switches; test_harness($(TEST_VERBOSE), '$(INST_LIB)', '$(INST_ARCHLIB)')" $(TEST_FILES)

testdb_static :: static pure_all $(MAP_TARGET)
	PERL_DL_NONLAZY=1 "/Users/melo/projects/cpan/mine/data-uuid-libuuid/$(MAP_TARGET)" $(MAP_PERLINC) "-I$(INST_LIB)" "-I$(INST_ARCHLIB)" $(TEST_FILE)
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Data-UUID-LibUUID" VERSION="0.06_01">' > Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="MIME::Base64" />' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Sub::Exporter" />' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" />' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::use::ok" />' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="asa::" />' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="darwin-thread-multi-2level-5.30" />' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Data-UUID-LibUUID.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Data-UUID-LibUUID.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'bench.pl' '$(INST_LIB)/Data/UUID/bench.pl' \
	  'lib/Data/UUID/LibUUID.pm' 'blib/lib/Data/UUID/LibUUID.pm' \
	  'lib/Data/UUID/LibUUID/DataUUIDCompat.pm' 'blib/lib/Data/UUID/LibUUID/DataUUIDCompat.pm' 
